      ...
      const mockReports: SupportReport[] = [
        {
          ...
          category: 'Bug Report',
          message:
            'The charts on the analytics page are flickering when I resize my window. Using Chrome on Windows.',
          status: 'pending',
        ...
        },
        {
          ...
          message:
            'I would love to see a dark mode toggle that automatically syncs with my OS settings.',
          status: 'resolved',
        ...
        },
        {
          ...
          message:
            'Cannot upgrade to Pro plan. The payment window closes instantly after clicking subscribe.',
          status: 'pending',
        ...
        {isLoading ? (
          ...
          filteredReports.map((report) => {
            ...
            return (
              <div
                key={report.id}
                className="card bg-white/[0.01] border-white/[0.05] hover:border-white/[0.1] transition-all overflow-hidden group"
              >
                  ...
                  <div className="flex-1 min-w-0 space-y-3">
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                      <div className="flex items-center gap-2">
                        ...
                        <h3 className="text-[13px] font-bold text-foreground/90">
                          {report.category}
                        </h3>
                        <span
                          className={`px-2 py-0.5 rounded-full text-[9px] font-bold uppercase tracking-tighter ${PRIORITY_COLORS[report.priority]}`}
                        >
                          {report.priority}
                        </span>
                      </div>
